...
from src.problem import add_trust_region


def get_tr_performance(graph, model, net, timeout=60):
    ...
    gaps = list()
    ns = list()
    for n in [0, 50, 100, 200, 500, len(x_hat)]:
    # for n in [0, 50, 100, 150]:
        ...
        model_ = add_trust_region(model_, sol_values, Delta=int(.05 * n))

        model_.setParam('limits/time', timeout)
        model_.optimize()

        if model_.getStatus().lower() not in ['optimal', 'timelimit']:
            print('trust region with n=',n,' made the optimizatio terminate with status ',model_.getStatus())
            break
...
if __name__ == '__main__':
    ...
    for graph, _, model in tqdm(ds):
        ...
        ns, runtimes, objs, gaps = get_tr_performance(graph, model, net, timeout=10)
    ...
    with open(f'scip_tr_performance_{wandb_run_id}.pkl', 'wb') as f:
